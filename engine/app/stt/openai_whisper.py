"""
OpenAI Whisper Cloud STT Engine (whisper-1).
"""
import os
import asyncio
...
class OpenAIWhisperEngine(BaseSTT):
    """
    OpenAI Cloud Whisper API (whisper-1).
    High accuracy multi-language transcription and timestamp generation.
    """

    def __init__(self, model_id: str = "openai-whisper"):
    ...
    def _transcribe_sync(self, audio_path: str, language: Optional[str] = None, **kwargs) -> Union[Dict[str, Any], str]:
        ...
        if not api_key:
            return "Error: OPENAI_API_KEY is not configured. Please define it in Models or Settings."
        ...
        logger.info(f"Transcribing via OpenAI Cloud Whisper ({audio_path})...")
        ...
        try:
            ...
            if resp.status_code != 200:
                logger.error(f"OpenAI Whisper API error (HTTP {resp.status_code}): {resp.text}")
                return f"Error: OpenAI Whisper API failed ({resp.status_code}): {resp.text}"
